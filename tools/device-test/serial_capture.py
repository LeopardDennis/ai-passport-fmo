#!/usr/bin/env python3
"""Capture FMO monitor USB serial logs on macOS using only Python's standard library."""
...
EVENTS = {
    "boot": re.compile(r"Starting FMO live monitor"),
    "ui_ready": re.compile(r"UI ready; LVGL peak="),
    "wifi_ip": re.compile(r"got ip:", re.IGNORECASE),
    "station_disconnect": re.compile(r"Station disconnected: reason="),
    "socket_issue": re.compile(r"(?:Events|Control) client creation failed"),
    "invalid_message": re.compile(r"Dropped invalid or oversized WebSocket message"),
    "heap_report": re.compile(r"Internal heap: min="),
    "network_failure": re.compile(r"FMO network task failed|Wi-Fi start failed"),
...
def main():
    ...
    output = args.output or Path("/tmp/fmo-device-logs") / (
        "fmo-serial-" + datetime.now().strftime("%Y%m%d-%H%M%S") + ".log"
    )
